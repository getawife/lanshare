...
const settings_path = path.join(app.getPath("userData"), "lanshare-settings.json");
let backend_process = null;
...
async function write_settings(settings) {
    await fs.mkdir(path.dirname(settings_path), { recursive: true });
    await fs.writeFile(settings_path, JSON.stringify(settings, null, 2), "utf8");
}
function apply_auto_start(enabled) {
    if (!app.isPackaged)
        return;
...
const backend_logs = [];
function append_backend_log(chunk) {
...
function analyze_backend_error() {
    ...
    if (full_log.includes("address may already be in use") ||
    ...
    if (full_log.includes("permission denied") ||
...
async function wait_for_backend() {
